<aside id="bo-sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-[#FCB861] text-[#6A2B09] border-r border-[#6A2B09]/25 transform -translate-x-full transition-transform duration-300 lg:translate-x-0 flex flex-col shrink-0 overflow-hidden">
    ...
    <nav class="px-3 py-4 space-y-1.5 flex-1">
        ...
        <a href="/backoffice/overview" class="sidebar-nav-link flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ request()->is('backoffice/overview') ? 'bg-[#6A2B09] text-[#FCB861]' : 'text-[#6A2B09] hover:bg-white/45' }}">
        ...
        <a href="/backoffice/booking" class="sidebar-nav-link flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ request()->is('backoffice/booking') || request()->is('backoffice/booking/*') ? 'bg-[#6A2B09] text-[#FCB861]' : 'text-[#6A2B09] hover:bg-white/45' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="sidebar-icon w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
            <span class="sidebar-label">Kelola Booking</span>
        </a>

        <a href="/backoffice/kelola_meja" class="sidebar-nav-link flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ request()->is('backoffice/kelola_meja') || request()->is('backoffice/daftar_meja') ? 'bg-[#6A2B09] text-[#FCB861]' : 'text-[#6A2B09] hover:bg-white/45' }}">
